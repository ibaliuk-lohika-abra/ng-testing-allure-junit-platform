 ...
 */
package com.tradeix.infrastructure.allure.junitplatform;
...
public class AllureJunitPlatform implements TestExecutionListener {
    ...
    private void startTestCase(final TestIdentifier testIdentifier) {
        final String prefix = AllureJunitPlatformUtils.getSuitePrefix();
        ...
        final TestResult result = new TestResult()
                .setUuid(uuid)
                .setName(prefix + testIdentifier.getDisplayName())
        ...
        testSource.flatMap(source -> {
            Optional<String> fullName = AllureJunitPlatformUtils.getFullName(source);
            Optional<String> res = Optional.empty();
            if (fullName.isPresent()) {
                res = Optional.of(String.format("%s%s", prefix, fullName.get()));
            }
            return res;
        }).ifPresent(result::setFullName);
        testSource.map(this::getSourceLabels).ifPresent(result.getLabels()::addAll);
        testClass.ifPresent(aClass -> {
            final String suiteName = prefix + getDisplayName(aClass).orElse(aClass.getCanonicalName());
    ...
    protected String getHistoryId(final TestIdentifier testIdentifier) {
        String prefix = AllureJunitPlatformUtils.getSuitePrefix();
        return prefix + md5(testIdentifier.getUniqueId());
    }
